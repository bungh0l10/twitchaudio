...
sub getChannel {
    ...
    if ($res->{success}) {
        ...
        if ($@ || !$data) {
            $log->warn("Invalid JSON response for channel $login");
            return $callback->();
        }

        $log->debug("getChannel response: " . encode_json($data));
...
sub getAudioUrl {
    ...
    if ($@ || !$data) {
        $log->warn("Invalid JSON token response for $channel");
        return;
    }

    $log->debug("token response: " . encode_json($data));
    ...
    $log->debug("m3u8 request: $playlistUrl");

    my $m3u = $http->get($playlistUrl);

    unless ($m3u->{success}) {
        $log->warn("Failed to fetch m3u for $channel");
        return;
    }
    ...
    for (my $i = 0; $i < @lines - 1; $i++) {
        if ($lines[$i] =~ /audio_only/ && $lines[$i + 1] =~ /^https/) {
            $log->debug("audio_only m3u8: $lines[$i+1]");
            return $lines[$i + 1];
        }
    }
